...
description: Cross-check live Clash Royale API payloads stored in raw_api_payloads against ~/Projects/clash-royale/cr-agent-api-docs/ and recommend concrete documentation patches — undocumented fields, new enum values, type or nullability drift, shape changes
---

# CR API Doc Audit

Read live API responses out of `elixir-v51.db` and compare them to the agent-facing reference in `~/Projects/clash-royale/cr-agent-api-docs/`. The goal is a tight, evidence-backed list of **doc patches worth making** — not a wall of field tables.
...
Source of truth for live behavior: `/Users/otto/Projects/clash-royale/elixir-bot/elixir-v51.db` → table `raw_api_payloads`.
...
conn = sqlite3.connect("file:/Users/otto/Projects/clash-royale/elixir-bot/elixir-v51.db?mode=ro", uri=True)
...
Outputs a list of interesting paths with coverage and observed types. Diff this against `~/Projects/clash-royale/cr-agent-api-docs/players.md`'s field table.
...
Match the existing doc voice — terse, table-driven, specific. Look at `~/Projects/clash-royale/cr-agent-api-docs/players.md` as the reference style:
...
Only edit `~/Projects/clash-royale/cr-agent-api-docs/*.md` when the user explicitly asks. When they do, apply only the patches they approved from the report — do not batch in cosmetic rewrites.
