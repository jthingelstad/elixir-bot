...
- `agent/pricing.py` — Canonical date-aware Anthropic token pricing shared by runtime spend charging, stored per-call receipts, `/status`, and the read-only operator report; do not copy rates into SQL or skills
...
a long-lived API-token user of Elixir MCP.** Phase 1 is LIVE
(2026-09-04): member trend (get_member include=trend), war attendance
(get_member_war_detail aspect=attendance), and the get_clan_standing
tool are served from Elixir MCP DIRECTLY — no shadow mode (Jamie);
local tables are the ERROR FALLBACK only. Client: `elixir_mcp.py`
(cr_api.py's error contract: None on any failure, never raises);
...
- Battle-intelligence views (archetypes, player-adjusted lift,
  ...
  capability is proven equivalent.
