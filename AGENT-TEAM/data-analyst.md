...
- **Drift sentinel (runtime):** the `api-sentinel` activity records first-seen schema paths and `/events` game-mode entries into `api_sentinel_observations`. Nothing in the product runtime evaluates them — the Operations Manager does, from `AGENT-TEAM/error-watch.md` step 5, which runs a 48h query for **structural** drift only (new schema path, progress key, or game mode — not routine new event tags; those are pure noise). That hand-off is deliberately thin: it says *something changed*, nothing more. **You own the rest** — characterize and quantify it, decide whether Elixir's model of the payload is now wrong, and file the `data` issue. Run `SELECT * FROM api_sentinel_observations ORDER BY first_seen_at DESC` for the full backlog the 48h window does not cover (the old `announced_signal_key IS NULL` clause was dropped in schema v35 — nothing ever wrote that column, so it filtered nothing), and don't wait to be handed one: the drift query is a floor, not your ceiling.
