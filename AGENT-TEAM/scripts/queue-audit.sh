...
gh issue list --state open --limit 100 --json number,title,labels,updatedAt \
  --jq '.[] | select((.labels|map(.name)) as $l | (($l|index("ready")) or ($l|index("approved"))) and (($l|index("wip"))|not) and (($l|index("blocked"))|not) and (($l|index("needs-design"))|not)) | "  #\(.number)  [\([.labels[].name]|join(","))]  \(.title)"'
...
gh issue list --state open --label wip --limit 100 --json number,title,labels,updatedAt \
  --jq '.[] | select(.updatedAt < "'"$cutoff"'") | "  #\(.number)  \(.title)  (updated \(.updatedAt[0:16]))"'
...
gh issue list --state open --limit 100 --json number,title,labels,updatedAt \
  --jq '.[] | select(.updatedAt < "'"$cutoff_d"'") | select((.labels|map(.name)) as $l | (($l|index("wip"))|not) and (($l|index("blocked"))|not) and (($l|index("needs-design"))|not)) | "  #\(.number)  [\([.labels[].name]|join(","))]  \(.title)"'
