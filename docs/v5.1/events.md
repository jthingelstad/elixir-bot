...
| `trophy_push` | `trophy_push:{tag}:{battle_dedup_key}` | a run of ≥3 consecutive positive-trophy competitive battles totaling ≥100; **anchored on the run's *last* battle** (carried semantics: `detectors.py:344–346` keys on `run[-1]`) | Carries `battle_trophy_push`; delta-scaled score in recognition.md. |
...
| `season_closed` | `season_closed:{season_id}` | `{final_rank, weeks, war_champ_tag, free_pass_tag, standings_top: […]}` | **New** — fires after Colosseum war days complete; computes Q2's honor + rotation outcome into `war_seasons`; the Q5 award pass consumes exactly this event. |
