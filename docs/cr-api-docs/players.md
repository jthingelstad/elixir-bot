...
| `name`                            | string        |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `expLevel`                        | integer       | **DEPRECATED (2026 Collection Level update).** Clash Royale removed Experience Level / King's Journey; the API still returns a legacy `expLevel` (and Elixir's ingest often stores 0) but it is NOT a live stat — do not surface it and do NOT derive King Tower from it. King Tower Level is now computed from card-upgrade counts (see `engine/king_tower.py` + memory cr-progression-model-2026); account progression is **Collection Level** (the `CollectionLevel` badge). |
| `kingTowerLevel`                  | integer       | Current King Tower Level. First observed 2026-09-02; 107 retained profile payloads across 27 members reported 12–16 through 2026-09-03. It matched Elixir's independent card-upgrade calculation for every sampled current profile, so retain the calculation as the source model and use this field as an API parity signal rather than a replacement authority. |
| `collectionLevel`                 | integer       | API stub when present: retained August 2026 payloads report only `0`. Do **not** use it as account progression; read `CollectionLevel` badge `progress` instead. |
| `expPoints`                       | integer       | DEPRECATED — legacy XP within current level (XP was removed from the game).                                                                                                                                                                                                                                                                                                                                                                                                     |
| `totalExpPoints`                  | integer       | DEPRECATED — legacy lifetime XP (XP was removed from the game).                                                                                                                                                                                                                                                                                                                                                                                                                 |
