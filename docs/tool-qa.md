...
| M6 | `get_river_race` · fame_today / top_fame_today | misleading | yes | Every fame_today=0 (fame_delta never populated: 0 nonzero across 644 rows). top_fame_today falls back to cumulative total → ranks by season total but is labeled "today". | Populate fame_delta or derive per-day from period-point/battle deltas; until then drop/null fame_today and rename top_fame_total. |
