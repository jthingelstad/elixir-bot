...
def test_record_and_query_incident():
    ...
    try:
        incidents.record_incident("test.component", ValueError("boom"),
                                   context={"tag": "#A"}, conn=conn)
        rows = incidents.open_incidents(conn)
        assert rows and rows[0]["component"] == "test.component"
        assert "boom" in rows[0]["summary"]
        assert rows[0]["detail"] and "ValueError" in rows[0]["detail"]  # traceback captured
...
def test_record_incident_never_raises():
    ...
    incidents.record_incident("test.safe", "just a string", conn=None)  # opens its own conn


def test_string_error_captures_caller_stack():
    conn = db.get_connection()
    try:
        incidents.record_incident("test.str", "manual message", severity="warn", conn=conn)
