...
# ----------------------------------------- stub calls must not reach the real file


def test_a_stub_call_is_refused_against_the_production_path(monkeypatch, caplog):
    """A probe script pointed at the repo without ELIXIR_TELEMETRY_DB_PATH writes
    fabricated rows into production telemetry. It had happened four times by
    2026-08-09, leaving 21 rows that skewed the averages this table exists for.

    The gap is unambiguous: the fastest real call ever recorded is 80.9ms, the
    slowest stub 0.57ms — no HTTP round trip finishes in under a millisecond."""
    monkeypatch.setattr(telemetry_store, "telemetry_path", lambda: telemetry_store.DEFAULT_PATH)
    called = []
    monkeypatch.setattr(telemetry_store, "connect", lambda: called.append(1))

    with caplog.at_level("WARNING", logger="elixir"):
        telemetry_store.record_llm_call("awareness", "claude-sonnet-5", duration_ms=0.05)

    assert not called, "a stub call must never reach the production telemetry file"
    assert any("stubbed client" in r.getMessage() for r in caplog.records)


def test_a_real_call_is_recorded_against_the_production_path(monkeypatch):
    """The guard must not swallow genuine traffic — 80.9ms is the real floor."""
    monkeypatch.setattr(telemetry_store, "telemetry_path", lambda: telemetry_store.DEFAULT_PATH)
    reached = []
    ...
    monkeypatch.setattr(telemetry_store, "connect", lambda: _Conn())
    telemetry_store.record_llm_call("awareness", "claude-sonnet-5", duration_ms=80.9)
    assert reached, "a real call must still be recorded"


def test_the_guard_does_not_apply_to_an_isolated_test_database():
    """Tests legitimately record stub calls and read them back. What is being
    protected is the production file, not the mechanism."""
    assert telemetry_store.telemetry_path() != telemetry_store.DEFAULT_PATH
    assert not telemetry_store._is_stub_call(0.05)
