...
def _member(conn, tag="#RAT", name="Ratko", *, promote="none", kick="none", demote="none",
            in_clan=True):
    conn.execute(
        "INSERT OR IGNORE INTO clans (clan_tag, name, first_seen_at, last_seen_at, is_home) "
        "VALUES ('#J2RGCRVG', 'POAP KINGS', ?, ?, 1)", (NOW, NOW))
    conn.execute(
    ...
    conn.execute(
        ...
        "VALUES (?, ?, 'test', ?)", (tag, NOW, None if in_clan else NOW))
    conn.execute(
...
def _case(conn, tag="#RAT", name="Ratko", *, case_type="promotion_review",
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (f"{case_type}:{tag}", case_type, status, f"{case_type}: {name}", tag, name,
         opened_at or _opened(9), NOW, NOW))
    conn.commit()
    return conn.execute("SELECT case_id FROM decision_cases WHERE case_key=?",
                        (f"{case_type}:{tag}",)).fetchone()["case_id"]
...
def _status(conn, case_id):
    return conn.execute("SELECT status FROM decision_cases WHERE case_id=?",
                        (case_id,)).fetchone()["status"]
...
def test_inactivity_case_uses_kick_state(engine_conn):
    ...
    cid = _case(engine_conn, tag="#IDL", name="Idle",
                case_type="inactivity_review", opened_at=_opened(9))
    out = cases.reconcile_uncorroborated_member_cases(now=NOW, conn=engine_conn)
    assert [c["case_id"] for c in out] == [cid]
    assert _status(engine_conn, cid) == "dismissed"
...
def test_kick_watch_state_is_not_dismissed(engine_conn):
    ...
    cid = _case(engine_conn, tag="#WCH", name="Watched",
                case_type="inactivity_review", opened_at=_opened(30))
    out = cases.reconcile_uncorroborated_member_cases(now=NOW, conn=engine_conn)
    assert out == []
    assert _status(engine_conn, cid) == "open"
...
def test_departed_member_left_to_departed_reconciler(engine_conn):
    ...
    cid = _case(engine_conn, tag="#GON", name="Gone",
                case_type="promotion_review", opened_at=_opened(30))
    out = cases.reconcile_uncorroborated_member_cases(now=NOW, conn=engine_conn)
    assert out == []
    assert _status(engine_conn, cid) == "open"
