...
def test_send_onboarding_message_uses_shared_sender():
    ...
    with (
        patch(
            "runtime.onboarding._onboarding_channel",
            new=AsyncMock(return_value=channel),
        ),
        patch(
            "runtime.onboarding.elixir_agent.generate_message",
            return_value="Welcome :elixir_happy:",
        ),
...
def test_handle_member_update_refreshes_roster_only_after_initial_no_match():
    ...
    with (
        ...
        patch(
            ...
            side_effect=[None, ("#ABC123", "King Levy")],
        ) as mock_match,
    ):
...
class TestRemoveMemberRoleForTag:
    ...
    def _run(self, *, identity, guild_member, member_role, manage_roles=True, forbidden=False):
        ...
        with (
            ...
            patch(
                "runtime.app._member_role_grant_status",
                return_value={"ok": manage_roles, "reason": "x"},
            ),
        ):
            return asyncio.run(
