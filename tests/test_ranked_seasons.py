...
podium event → pol_champ awards → chronicle → idempotence), season-id math
...
def _player_payload(
    tag_suffix: str, name: str, *, league, trophies, rank=None, last=None, best=None
) -> dict:
    p = {
        "tag": f"#{tag_suffix}",
        "name": name,
...
def test_ultimate_champion_fires_at_league_7():
    ...
    try:
        ...
        _emit_player(
            ...
            league=6,
            trophies=1500,
            last={"leagueNumber": 5, "trophies": 1400},
        )
        _emit_player(
            conn,
            "UC1",
...
def test_shape_change_alone_emits_nothing():
    ...
    try:
        ...
        set_baseline(
            ...
            "#SHAPE",
            "ranked",
            {"league": 7, "rank": None, "trophies": 1867},
            AT_JUNE,
        )
        n = 0
        from engine.emitters.player import project_player_aspects
...
def test_rollover_closes_season_and_posts_podium():
    ...
    try:
        ...
        _emit_player(
            ...
            league=7,
            trophies=1900,
            last={"leagueNumber": 6, "trophies": 1500},
        )
        _emit_player(
            conn,
            "ATT",
        ...
        _emit_player(
            ...
            league=2,
            trophies=None,
            last={"leagueNumber": 1, "trophies": None},
        )
        _emit_player(
            conn,
            "MAS",
        ...
        season = conn.execute(
            "SELECT * FROM pol_seasons WHERE pol_season_id = '2026-06'"
        ).fetchone()
        ...
        # ONE podium event for the current awareness pipeline.
        events = conn.execute(
            "SELECT * FROM clan_events WHERE event_type = 'pol_season_podium'"
        ).fetchall()
        assert len(events) == 1
        payload = json.loads(events[0]["payload_json"])
        assert payload["podium"][0]["name"] == "Atternam"
        from runtime.awareness.read import HARD_POST_EVENT_TYPES, _compact_signal

        assert "pol_season_podium" in HARD_POST_EVENT_TYPES
        compact = _compact_signal(
            {
                "event_type": "pol_season_podium",
                "stream": "clan",
                "dedup_key": events[0]["dedup_key"],
                "payload": payload,
            }
        )
        assert compact["pol_season_id"] == "2026-06"
        assert compact["podium"][0]["name"] == "Atternam"

        # pol_champ awards under the sortable integer season id
        champs = conn.execute(
            "SELECT player_tag, rank FROM awards WHERE award_type = 'pol_champ' "
            "AND season_id = 202606 ORDER BY rank"
        ).fetchall()
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM clan_events WHERE event_type = 'pol_season_podium'"
            ).fetchone()[0]
            == 1
        )
        assert (
...
def test_teammate_extracted_from_2v2():
    ...
    solo = [
        {
            "type": "PvP",
            "battleTime": "20260704T120100.000Z",
