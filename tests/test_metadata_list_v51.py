...
def _seed_member(conn, tag, name, *, active=True, role="member", clan_rank=1):
    conn.execute(
        ...
        "VALUES (?,?,?,?)",
        (tag, name, "2026-01-01T00:00:00", "2026-07-01T00:00:00"),
    )
    conn.execute(
        "INSERT INTO player_current_state (player_tag, observed_at, role, clan_rank) "
        "VALUES (?,?,?,?)",
        (tag, "2026-07-01T00:00:00", role, clan_rank),
    )
    conn.execute(
        "INSERT INTO player_metadata (player_tag, joined_at, birth_month, birth_day, "
...
def test_list_member_metadata_rows_reads_v51_and_filters_active():
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO discord_users (discord_user_id, username, global_name, display_name, "
            "first_seen_at, last_seen_at) VALUES ('111','ada_cr','Ada','Ada CR','x','x')"
        )
        conn.execute(
            "INSERT INTO discord_links (discord_user_id, player_tag, linked_at, source, "
