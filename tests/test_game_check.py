...
def test_clean_maxed_card_passes():
    assert g.check_post("just maxed Balloon to level 16", {"card_name": "Balloon", "milestone": 16}) == []
...
def test_road_arena_up_passes():
    assert g.check_post("moved up to Boot Camp", {"arena_id": 54000010, "arena_name": "Boot Camp"}) == []
...
def test_fabricated_card_name_flags(engine_conn):
    ...
    out = g.check_post("maxed Zorptron to 16", {"card_name": "Zorptron", "milestone": 16}, engine_conn)
...
def test_real_card_passes_catalog(engine_conn):
    ...
    assert g.check_post("Ice Spirit to 16", {"card_name": "Ice Spirit", "milestone": 16}, engine_conn) == []


def test_empty_catalog_skips_card_name_check(engine_conn):
    # no cards seeded → cannot judge names → no false-positive
    assert g.check_post("maxed Whatever to 16", {"card_name": "Whatever", "milestone": 16}, engine_conn) == []
