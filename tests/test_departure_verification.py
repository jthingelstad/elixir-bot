...
def _seed_departure(
    ...
    tag="#A",
    name="Alice",
    left_at="2026-07-12T11:00:00Z",
    joined_at="2026-05-01",
    leave_source="roster_diff",
):
    conn.execute(
...
def _open_departure_card(conn, tag="#A"):
    return conn.execute(
        ...
        "AND target_player_tag=? AND status='proposed'",
        (tag,),
    ).fetchone()
...
def _leave_source(conn, tag="#A"):
    return conn.execute(
        "SELECT leave_source FROM clan_memberships WHERE player_tag=? "
...
def test_classify_leave_writes_authoritative_source_and_memory(
    ...
    updated = classify_departure(
        ...
        discord_user_id=42,
        comment="left for a friend's clan",
        conn=engine_conn,
    )
...
def test_classify_kick_sets_verified_kick(engine_conn, _isolate_default_sqlite_db):
    ...
    card = _open_departure_card(engine_conn)
    classify_departure(
        card["action_id"], classification="kick", discord_user_id=42, conn=engine_conn
    )
    assert _leave_source(engine_conn) == "leader_verified_kick"
...
def test_verified_leave_emits_event_kick_does_not(engine_conn, _isolate_default_sqlite_db):
    ...
    assert n == 1
    ev = engine_conn.execute(
        "SELECT event_type FROM clan_events WHERE subject_tag='#A' "
