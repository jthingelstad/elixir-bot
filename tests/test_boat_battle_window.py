...
def _boat(conn, season, section, n, outcome, start_idx=0):
    for i in range(n):
        conn.execute(
            ...
            (f"{season}-{section}-{outcome}-{start_idx+i}", f"2026070{section}T1{i:02d}000.000Z",
             outcome, season, section),
        )
...
def test_boat_record_scopes_to_recent_wars():
    ...
    try:
        conn.execute("INSERT INTO players (player_tag, current_name, first_seen_at, last_seen_at) VALUES ('#B1','B','2026-06-01','2026-07-11')")
