...
def _releases(conn):
    return list_memories(
        viewer_scope="system_internal", include_system_internal=True,
...
def test_upsert_release_memory_records(engine_conn):
    mem = upsert_release_memory(
        name="Blazing Balloon", date="2026-07-08", tag="blazing-balloon",
        subject="Shiny", body="I learned to shine.",
        url="https://github.com/x/y/releases/tag/blazing-balloon", conn=engine_conn)
...
def test_upsert_release_memory_is_idempotent(engine_conn):
    upsert_release_memory(name="Blazing Balloon", date="2026-07-08",
                          tag="blazing-balloon", body="v1", conn=engine_conn)
    upsert_release_memory(name="Blazing Balloon", date="2026-07-08",
                          tag="blazing-balloon", body="v2 — updated", conn=engine_conn)
    rows = _releases(engine_conn)
