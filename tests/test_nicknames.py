...
@pytest.mark.parametrize("name,flagged", [
    ...
    ("²⁸", False), ("Sebastián", False), ("Ｓｈａｆｉｔｈ Ｎｉｈａｌ♥️", False),
    ("L-Drxgo⚡", False), ("kiruba⚜️", False), ("King Thing", False),
])
...
def _roster(members):
    return {"tag": "#J2RGCRVG", "name": "POAP KINGS", "clanScore": 60000,
            "clanWarTrophies": 900,
            "memberList": [{"tag": t, "name": n, "role": "member", "trophies": 5000,
                            "donations": 0} for t, n in members]}


def _emit_roster(conn, members, at):
    conn.execute("INSERT OR IGNORE INTO clans (clan_tag, name, first_seen_at, "
                 "last_seen_at, is_home) VALUES ('#J2RGCRVG','POAP KINGS','2026-02-04',?,1)", (at,))
    return emit(conn, "clan", "#J2RGCRVG", "roster",
                project_clan_aspects(_roster(members))["roster"], at)
...
def test_residual_member_named_before_member_joined_posts(engine_conn):
    ...
    payload = json.loads(c.execute(
        ...
        "AND subject_tag='#DOTS'").fetchone()["payload_json"])
