...
def test_member_note_memory_reads_players_not_members():
    ...
    try:
        ...
        mem = cm.upsert_member_note_memory(
            member_tag="#M1", member_label="Marco",
            note="Reliable war attacker; never misses a day.",
            created_by="leader:jamie", conn=conn,
        )
...
def test_race_streak_skips_null_rank_training_week():
    ...
    conn = _mem_conn()
    try:
        conn.execute("INSERT INTO war_seasons (season_id, started_at) VALUES (133,'2026-06-01'), (134,'2026-07-01')")
        rows = [
            (133, 0, None, 1), (133, 1, None, 1), (133, 2, None, 1),
            (133, 3, None, 1), (133, 4, "colosseum", 1),   # 5 straight 1sts
        ...
        for season, section, ptype, rank in rows:
            conn.execute(
                ...
                "VALUES (?,?,?,?)", (season, section, ptype, rank))
        conn.commit()
