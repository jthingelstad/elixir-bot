...
def _seed_free_pass_case(conn, *, last_free_pass):
    ...
    conn.execute(
        "INSERT OR IGNORE INTO war_seasons (season_id, started_at) VALUES (139, '2026-01-01')"
    )
    conn.execute(
    ...
    for tag, pts in (("#ACE", 6000), ("#BEE", 4000), ("#CID", 2000)):
        conn.execute(
            ...
            "VALUES (140, 0, ?, ?, 16, '2026-02-15T00:00:00Z')",
            (tag, pts),
        )
    if last_free_pass:
...
def test_free_pass_rotates_off_last_months_holder(engine_conn):
    ...
    row = engine_conn.execute(
        "SELECT war_champ_tag, free_pass_tag FROM war_seasons WHERE season_id=140"
    ).fetchone()
...
def test_award_race_lead_change_emits_once_per_new_leader(engine_conn):
    ...
    old = {
        "season_id": 140,
        "war_champ_leader": {"tag": "#ACE", "name": "Ace", "points": 3000},
    }
    new = {
        "season_id": 140,
        "war_champ_leader": {"tag": "#BEE", "name": "Bee", "points": 3500},
    }
...
def test_reset_then_rollover_finalizes_from_peak(engine_conn):
    ...
    part = c.execute(
        ...
        "WHERE season_id=133 AND section_index=4"
    ).fetchone()
...
def test_battle_day_one_index_zero_fires_and_records(engine_conn):
    ...
    emit(
        ...
        TAG,
        "race",
        _race_day(0, 17, "warDay", 300, 3),
        "2026-08-01T11:00:00Z",
    )
    ev = c.execute(
        "SELECT payload_json FROM war_events WHERE event_type='war_day_opened'"
    ).fetchone()
...
def test_attendance_day_never_zeroed_by_reset(engine_conn):
    ...
    emit(
        ...
        TAG,
        "race",
        _race_day(0, 17, "warDay", 300, 4),
        "2026-08-01T13:00:00Z",
    )  # 4 decks in
    before = c.execute(
        "SELECT decks_used FROM war_attendance_days WHERE season_id=140 AND "
        "section_index=0 AND war_day_index=0 AND player_tag='#AAA'"
    ).fetchone()["decks_used"]
    assert before == 4
...
def test_monotonic_participation_never_decreases(engine_conn):
    ...
    before = c.execute(
        ...
        "WHERE season_id=133 AND section_index=4 AND player_tag='#AAA'"
    ).fetchone()
