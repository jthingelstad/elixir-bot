...
def _raise(conn):
    intent_id = delivery.raise_intent(
        ...
        {
            "subject_tag": "#A",
            "event_type": "collection_level_milestone",
            "player_name": "Alice",
            "milestone": 1700,
            "collection_level": 1712,
        },
        NOW,
    )
    conn.commit()
...
def test_pipeline_delivers_composed_copy(legacy_engine_conn):
    iid = _raise(legacy_engine_conn)
    counters, sent = _run(legacy_engine_conn, lambda i: "Alice hit level 45 — nice.")
...
def test_pipeline_meta_copy_uses_deterministic_fallback(legacy_engine_conn):
