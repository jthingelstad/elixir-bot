...
def seeded_db(engine_conn, _isolate_default_sqlite_db):
    ...
    c.execute(
        "INSERT INTO war_weeks (season_id, section_index, our_rank, our_fame)"
        " VALUES (132, 0, 1, 10000)"
    )
    c.execute(
        "INSERT INTO war_participation (season_id, section_index, player_tag,"
        " fame, decks_used, observed_at) VALUES (132, 0, '#A', 3000, 16, ?)",
        (NOW,),
    )
    c.execute(
        "INSERT INTO war_attendance_days (season_id, section_index, war_day_index,"
        " player_tag, decks_used, decks_available, observed_at)"
        " VALUES (132, 0, 0, '#A', 4, 4, ?)",
        (NOW,),
    )
    c.execute(
        "INSERT INTO awards (award_type, season_id, player_tag, rank, awarded_at)"
        " VALUES ('war_champ', 132, '#A', 1, ?)",
        (NOW,),
    )
    c.execute(
        "INSERT INTO awards (award_type, season_id, player_tag, rank, awarded_at)"
        " VALUES ('free_pass', 132, '#A', 1, ?)",
        (NOW,),
    )
    c.execute(
        ...
        " 'collection_level_milestone', '#A', ?, '{}', ?)",
        (NOW, NOW),
    )
    c.execute(
        "INSERT INTO state_baselines (entity_kind, entity_tag, aspect, payload_json,"
...
def test_get_clan_voyage_removed():
    ...
    names = json.dumps(getattr(tool_defs, "TOOLS", []) or [t for t in dir(tool_defs)])
