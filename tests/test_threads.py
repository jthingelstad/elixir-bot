...
def test_war_day_intent_carries_thread_and_week_finished_does_not():
    ...
    try:
        ...
        iid = delivery.raise_intent(
            ...
            "river-race",
            "public",
            {"event_type": "war_day_opened"},
            "2026-07-05T10:00:00Z",
            thread_id=wk["thread_id"],
        )
        row = conn.execute(
            "SELECT * FROM communication_intents WHERE intent_id=?", (iid,)
        ).fetchone()
        ...
        row2 = conn.execute(
            "SELECT * FROM communication_intents WHERE intent_id=?", (iid2,)
        ).fetchone()
