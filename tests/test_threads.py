...
def test_game_event_watcher_born_then_locked(monkeypatch):
    ...
    try:
        ...
        conn.close()
    asyncio.run(app._watch_game_event_threads(222, db.get_connection))
    conn = db.get_connection()
    try:
        ...
        conn.close()
    bot.threads[thread_id] = _FakeThread(thread_id)
    asyncio.run(app._watch_game_event_threads(222, db.get_connection))
    conn = db.get_connection()
    try:
        row = conn.execute("SELECT locked_at FROM event_threads WHERE mode_name='Event_TestFest'").fetchone()
