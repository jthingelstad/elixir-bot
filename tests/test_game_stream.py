...
def _card(cid, name, rarity="common", elixir=3, icon="http://x/c.png"):
    return {"id": cid, "name": name, "rarity": rarity, "elixirCost": elixir,
            "maxLevel": 14, "iconUrls": {"medium": icon}}


def _obs(conn, sentinel_type, name, *, entity="2G2RPVPP", first_seen="2026-07-07T14:00:00",
         sample=None):
    conn.execute(
        """INSERT INTO api_sentinel_observations
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (sentinel_type, "player.badges" if sentinel_type == "badge_name" else "events",
         name, "player", entity, first_seen, first_seen,
         json.dumps(sample or {}), first_seen, first_seen),
    )
...
def test_card_sync_detects_new_card_not_bootstrap_and_is_idempotent():
    ...
    sync_card_catalog({"items": [_card(26000000, "Knight"),
                                 _card(28000018, "Ronin", "legendary")]}, conn=conn)
...
def test_emitter_skips_mastery_attributes_novel_badge_and_reads_events():
    ...
    _obs(conn, "badge_name", "OldBadge", first_seen="2026-06-13T11:46:39",
    ...
    _obs(conn, "badge_name", "Chaos_S2", first_seen="2026-07-07T15:00:00",
         sample={"badge": {"name": "Chaos_S2", "iconUrls": {"large": "http://x/c.png"}}})
    _obs(conn, "badge_name", "MasteryRonin", first_seen="2026-07-07T15:01:00")
    _obs(conn, "event", "#E9", entity="global", first_seen="2026-07-07T15:02:00",
         sample={"eventTag": "#E9", "title": "Mega Chaos", "description": "2v2 chaos"})
    conn.commit()
    assert emit_game_from_sentinel(conn, "2026-07-07T15:05:00Z") == 2  # badge + event; Mastery skipped
...
def test_emitter_unattributed_when_member_unresolvable():
    ...
    _obs(conn, "badge_name", "GhostBadge", entity="ZZZUNKNOWN", first_seen="2026-06-01",
         sample={"badge": {"name": "GhostBadge"}})
    conn.commit()
    emit_game_from_sentinel(conn, "2026-07-07T14:00:00Z")  # seed pass
    _obs(conn, "badge_name", "LoneBadge", entity="ZZZUNKNOWN", first_seen="2026-07-07T15:00:00",
...
def test_recognizer_groups_by_change_key_and_is_idempotent():
    ...
    ge.insert_game_event(conn, dedup_key="event_started:#E1", event_type="event_started",
                         change_key="event:#E1", observed_at="2026-07-07T00:00:00Z",
                         payload={"event_type": "event_started", "title": "Chaos"})
    ge.insert_game_event(conn, dedup_key="event_started:#E1:mode", event_type="event_started",
                         change_key="event:#E1", observed_at="2026-07-07T00:00:01Z",
    ...
    intents = conn.execute("SELECT * FROM communication_intents WHERE lane='announcements'").fetchall()
...
def test_recognizer_never_posts_backfilled_rows():
    conn = _mem()
    ge.insert_game_event(conn, dedup_key="card_added:1", event_type="card_added",
                         change_key="card:1", observed_at="2026-01-01T00:00:00Z",
...
def _render(intent_type, payload):
    return compose.render_intent(
        {"intent_type": intent_type, "scope": "public", "payload_json": json.dumps(payload)})


def test_fallbacks_read_cleanly_for_each_game_type():
    card = _render("game:card_added", {"event_type": "card_added", "name": "Ronin",
                                       "rarity": "legendary", "elixir_cost": 3})
    assert "Ronin" in card and "Legendary" in card and "3 elixir" in card
    ev = _render("game:event_started", {"event_type": "event_started", "title": "Mega Chaos",
                                        "description": "2v2 chaos"})
    assert "Mega Chaos" in ev
    badge = _render("game:event_badge_earned", {"event_type": "event_badge_earned",
                    "badge_label": "Chaos S2", "member_name": "Aaqib Javed"})
    assert "Chaos S2" in badge and "Aaqib Javed" in badge
    anon = _render("game:event_badge_earned", {"event_type": "event_badge_earned",
                   "badge_label": "Chaos S2", "member_name": None})
...
def test_card_intent_delivers_with_image_but_text_lanes_do_not():
    ...
    sync_card_catalog({"items": [_card(1, "Knight"),
...
def test_text_only_send_fn_never_sees_image_kwarg():
    conn = _mem()
    ge.insert_game_event(conn, dedup_key="card_added:9", event_type="card_added",
                         change_key="card:9", observed_at="2026-07-07T00:00:00Z",
                         payload={"event_type": "card_added", "name": "X",
...
def test_chaos_s2_replay_end_to_end():
    ...
    _obs(conn, "badge_name", "BaselineBadge", first_seen="2026-06-13T11:46:39",
         sample={"badge": {"name": "BaselineBadge"}})
    conn.commit()
    emit_game_from_sentinel(conn, "2026-07-06T00:00:00Z")  # go-live seed → nothing
    _obs(conn, "badge_name", "Chaos_S2", first_seen="2026-07-06T21:30:43",
