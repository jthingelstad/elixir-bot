...
def main() -> int:
    ...
    for r in releases:
        ...
        url = (f"https://github.com/jthingelstad/elixir-bot/releases/tag/{r['tag']}"
               if rn._git(["tag", "-l", r["tag"]]).strip() else None)
        mem = upsert_release_memory(name=r["name"], date=r["date"], tag=r["tag"],
                                    body=r["body"], url=url, metadata=meta)
