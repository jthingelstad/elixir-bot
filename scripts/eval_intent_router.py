...
CATEGORIES = [
    (
        "clan_ops",
        "leader-style operations questions: kicks, promotions, who joined, contributors, war participation, roster review",
    ),
    (
        "clan_member",
        "regular member questions about other members or the clan: who is doing well, donations, wars left, who's new",
    ),
    (
        "own_deck",
        "questions about the speaker's own deck: review, suggest, what's in it, war decks, swap suggestions",
    ),
    ("other_deck", "questions about another player's deck (by name or @mention)"),
    (
        "trophy_road",
        "trophy road, arenas, leagues, climbing, pathing of evolution unlocks, rewards",
    ),
    (
        "general_cr",
        "general Clash Royale gameplay: card matchups, archetypes, meta, tips, mechanics",
    ),
    ("help_meta", "what can the bot do, how to use it, what commands exist"),
    (
        "chat_noise",
        "casual chat: thanks, lol, agreement, off-topic — bot should usually NOT respond",
    ),
...
def generate_questions(
    per_category: int = 5, model: str | None = None, seed_round: int = 1
) -> list[dict]:
...
def report(rows: list[dict], round_num: int) -> dict:
    ...
    for r in low_conf[:8]:
        print(
            f"  conf={r.get('confidence'):.2f} {r['category']:13s} → {r['route']:22s} q={r['question'][:70]!r}"
        )
