#!/usr/bin/env python3
"""Print recent prompt failures for review."""
...
def _print_failure(row: dict, *, include_raw: bool) -> None:
    print(
        f"[{row['failure_id']}] {row['recorded_at']} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Review recent Elixir prompt failures.")
    ...
    rows = db.list_prompt_failures(limit=args.limit, workflow=args.workflow)
    if args.json:
        print(json.dumps(rows, indent=2, ensure_ascii=False))
        return
    if not rows:
        print("No prompt failures recorded.")
        return
    for row in rows:
        _print_failure(row, include_raw=args.raw)
