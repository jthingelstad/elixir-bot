...
2. Gather signal since the last run:
   ...
   * Recurring questions, friction, or unmet asks in stored Discord message history and `#leaders`.
3. Groom the discovery backlog before filing anything new:
   * Dedupe overlapping `quality`, `data`, `proposal`, `needs-design`, and `blocked` issues.
   * Close or relabel stale generated issues whose evidence no longer matches current v5/Event Core behavior.
   * Surface proposals still waiting for Jamie's approval.
4. Ask the discovery questions: What should Elixir have noticed and didn't? Which recommendations were valuable vs. ignored, and why? What patterns are emerging? What capability is missing?
5. Run each candidate through the Decision Filter above. Discard the ones that fail. Dedupe against existing issues.
6. File at most a few high-quality proposals, each with: the problem, the mission goal it serves, the evidence, the smallest valuable version, and a clear acceptance criterion. **Every new-direction issue gets the `proposal` label plus a type label** so it routes correctly once approved:
