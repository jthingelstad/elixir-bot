...
1. Update the repository if the worktree is clean. If it is dirty, stop and open an issue describing the unexpected state.
...
7. Commit directly to main with the issue reference (`Closes #N` / `Refs #N`). Update the issue: what changed, test evidence, and whether a deploy is required.
