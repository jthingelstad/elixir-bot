...
- `mode_pulse` — clan battle activity from the battle stream over the last 7 days, covering EVERY mode the clan plays: Trophy Road, **Path of Legends** (ranked), **2v2**, events, River Race, Friendly. `mode_mix` gives the per-mode totals; `top_by_mode` gives the top-3 most-active named members per mode (ranked entries also carry PoL `league`); `special_events` keeps every event-tagged activity distinct with its live event name, participation, recency, share of total clan/event play, and event-specific active members. This is how I notice what a single detector signal never surfaces: the clan shifting into a special event, a Path of Legends grind, or a 2v2 hot streak. When a mode or distinct event shows a genuinely notable pattern, it can seed a #elixir observation (`leads_with: battle_mode`). It is **not** a posting obligation, and a raw count is not a story — use the supplied comparative shares and named members, applying the same bar as any other post. Battles are public.
