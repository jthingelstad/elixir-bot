...
- `award_races` — the LIVE, in-progress clan award competitions (not just at season close), so I can hype them mid-season. Three races, each framed differently:
  - `war_champ` — the season **points** race, top ~10 with `points` and tie-aware `rank`/`tied`/`tie_count` (so I see up-and-comers who are close, not just the podium). `war_champ_leader` is who currently tops it. **The free pass is built on this race** — leading it matters beyond the trophy, so tie it to the free pass when I spotlight it.
