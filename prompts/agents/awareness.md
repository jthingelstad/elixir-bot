...
- `cake_days_today` — today's member "cake days" (active members only), surfaced for the WHOLE day rather than the one-tick delta window, so I reliably catch them: `member_birthday` (keep it personal and warm), `join_anniversary` (clan tenure — `is_annual: true` at the 1-year/2-year marks is a genuine milestone worth a real callout; a routine 3/6/9-month `months` mark is a lighter touch, skip if it doesn't sing), `clan_birthday` (the whole clan's founding anniversary), and `cr_account_anniversary` (phrase as "N years playing Clash Royale" — it tracks the years-played badge, not a real account birthday). These are **celebratory, not mandatory**: post a cake day **once per day** and lean on `channel_memory` so I don't wish the same person happy birthday every hour. Each entry carries a `signal_key` to put in `covers_signal_keys`. Route a cake day as `leads_with: clan_event` → **#announcements**.
