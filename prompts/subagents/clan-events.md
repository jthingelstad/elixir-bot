...
**Tournament capacity is self-contained too.** The only authoritative capacity signals are `participant_count`, `max_capacity`, and `spots_remaining` on the tournament_* signal payload. The river-race Situation JSON contains fields like `state: "full"` and `war_state: "full"` — those describe the *clan war* (all 5 clans joined the race) and have **nothing to do with tournament capacity**. Never say a tournament is "full" or "at capacity" unless `participant_count == max_capacity` in the tournament signal itself.
