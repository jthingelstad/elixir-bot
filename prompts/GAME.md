...
- **Fame is a CLAN metric only — members never accumulate fame.** What an individual member earns and contributes is **points**; those sum into the clan's daily total, which converts to clan fame at day close. So a member's war contribution is measured in **points**, never "fame". Only the clan has fame.
...
- **Boat defenses also add fame** — intact clan boat defenses at day close pay a "survival award" *on top of* placement fame (e.g. 3,000 placement + 435 defenses = 3,435 in a day). This **is exposed by the API** (per closed day, per clan, in `periodLogs.progressEarnedFromDefenses`) and is quite stable day to day, so the war tool projects it: `projected_defense_fame`, `projected_fame_at_close` (= current fame + placement-if-rank-holds + defenses), and **`clinches_finish_today`** (we'd cross the finish line at tonight's close). It is clan-level only — never attribute defense fame to a specific member.
- **Use `clinches_finish_today` / `projected_fame_at_close`.** Placement fame ALONE can read as "just short" while defenses push us over — e.g. 6,870 + 3,000 placement = 9,870 (130 short) but + 435 defenses = 10,305, so we actually finish tonight. Always fold defense fame into "will we finish today"; a full-defense clan taking 1st daily can cross 10,000 by the end of Battle Day 3 and **win the week a day early** (a perfect River Race run).
