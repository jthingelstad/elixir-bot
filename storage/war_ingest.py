...
def upsert_war_current_state(war_data, conn=None):
    ...
    season_id = infer_current_season_id_from_live_state(war_data, latest_logged_race)
    section_index = war_data.get("sectionIndex")
    period_index = war_data.get("periodIndex")
    phase = resolve_phase(war_data.get("periodType"), period_index)
    day_number = phase_day_number(phase, period_index)
    battle_date = war_day_key(season_id, section_index, period_index, observed_at)
    for participant in clan.get("participants", []):
        ...
        conn.execute(
            ...
            (member_id, battle_date, observed_at, participant.get("fame", 0), participant.get("repairPoints", 0), participant.get("boatAttacks", 0), participant.get("decksUsed", 0), participant.get("decksUsedToday", 0), season_id, section_index, period_index, phase, day_number, _json_or_none(participant)),
        )
        conn.execute(
