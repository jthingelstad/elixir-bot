...
def send(*, to: list[str] | str, subject: str, body: str, sign: bool = True,
    ...
    html_body = (email_render.text_to_html(body, signature_html=_HTML_SIG if sign else None)
                 if email_jmap.HTML_ENABLED else None)
    return email_jmap.send_email(
        to=to, subject=subject, body=text_body, html_body=html_body, cc=cc, bcc=bcc,
