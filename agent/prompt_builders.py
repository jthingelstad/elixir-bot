...
def _deck_review_system(channel_name, *, mode: str = "regular", subject: str = "review"):
    ...
    base_guidance = (
        ...
        "When a member asks what deck to try NEXT, for WAR DECKS, or for a deck built AROUND a card, call get_deck_recommendations — it is the only tool that gates suggestions on what they own and can field at level. Views: discover / war_set / anchored (anchored needs 'card'). War decks may not reuse a card across the four, which is why war_set exists; do not hand-assemble four decks yourself. "
