...
def _deck_review_system(channel_name, *, mode: str = "regular", subject: str = "review"):
    ...
    base_guidance = (
        ...
        "When a member asks what to UPGRADE, what deck to try NEXT, for WAR DECKS, or for a deck built AROUND a card, call get_deck_recommendations — it is the only tool that gates suggestions on what they own and can field at level. Views: upgrades / discover / war_set / anchored (anchored needs 'card'). War decks may not reuse a card across the four, which is why war_set exists; do not hand-assemble four decks yourself. "
        "It deliberately returns NO win rates: clan deck win rates measure who played a deck as much as the deck, and do not transfer between members. Never present fielded_by_members as evidence a deck is good, and never pair a recommended deck with a win rate from elsewhere. When upgrades reports all_played_cards_maxed, say there is nothing to upgrade rather than inventing one. meta_snapshot entries come from a dated web-search snapshot — cite it as current meta, not as clan evidence.\n\n"
    )
    ...
    if subject == "suggest":
        subject_guidance = (
            ...
            "Each war deck should have a distinct role and a clear archetype name — e.g. a beatdown anchor, a control deck, a cycle/chip deck, a siege deck. Avoid four variants of the same archetype. "
...
def _member_report_system():
    return _build_system_prompt(
        ...
        "your battle_intro paragraphs, your progress milestones as a bullet list, and a 'Your "
        "matchups / How close it was / Elixir wasted' block — so you narrate and hype, you do NOT "
        "list stats or enumerate battles.\n\n"
        "THE COACHING FACTS (HOW CLOSE, ELIXIR, MATCHUPS in the brief) are the most useful things "
        ...
        "  * TOUGHEST MATCHUPS are cards that BEAT them; BEST MATCHUPS are cards they HANDLE. "
        "Reversing these turns encouragement into an insult.\n"
