...
def _war_chronicle(conn, season_id: int) -> str | None:
    ...
    joins = _rows(
        ...
        WHERE cm.joined_at >= s.started_at
          AND (s.ended_at IS NULL OR cm.joined_at <= s.ended_at)""",
        (season_id,),
    )
...
def _ranked_chronicle(conn, season_id: str) -> str | None:
    ...
    players = _rows(
        ...
        ORDER BY r.league DESC, COALESCE(r.rating,0) DESC""",
        (season_id,),
    )
    if not players:
...
def write_season_chronicle(conn, kind: str, season_id, observed_at: str) -> int | None:
    ...
    try:
        ...
        memory_store.attach_tags(
            mem["memory_id"],
            ["chronicle", tag_key],
            actor="engine.chronicles",
            conn=conn,
        )
        log.info(
            "chronicle written: %s season %s (memory %s)",
            kind,
            season_id,
            mem["memory_id"],
        )
        ...
        log.exception("chronicles.write failed: kind=%s season_id=%s", kind, season_id)
        return None
