...
WAR_LEAGUE_TIERS = (
    (5000, "Legendary League III"),
    (4000, "Legendary League II"),
    (3000, "Legendary League I"),
    (2500, "Gold League III"),
    (2000, "Gold League II"),
    (1500, "Gold League I"),
    (1200, "Silver League III"),
    (900, "Silver League II"),
    (600, "Silver League I"),
    (400, "Bronze League III"),
    (200, "Bronze League II"),
    (0, "Bronze League I"),
)
...
def _emit_donation_reset(
        ...
        (
            {
                "tag": tag,
                "name": (m or {}).get("name"),
                "donations": (m or {}).get("donations") or 0,
            }
    ...
    return _emit(
        ...
        f"weekly_donation_leader:{week_key}",
...
def emit_clan_entity(conn, clan_tag, old, new, observed_at, window_start) -> int:
    ...
    if (
        ...
        for milestone in range(first, new_score + 1, CLAN_SCORE_MILESTONE_STEP):
            n += _emit(
                ...
                observed_at,
                window_start,
                "clan_score_milestone",
                f"clan_score_milestone:{clan_tag}:{milestone}",
    ...
    if old_league and new_league and old_league != new_league:
        n += _emit(
            ...
            window_start,
            "clan_league_changed",
            f"clan_league_changed:{clan_tag}:{new_league}",
            {
                "league": new_league,
                "prev_league": old_league,
...
def emit_calendar(conn, today_chicago: str) -> int:
    ...
    for r in conn.execute(
        ...
        n += cal_event(
            "member_birthday",
            f"member_birthday:{r['tag']}:{today_chicago}",
            r["tag"],
            {"name": r["name"]},
        )
    ...
    for r in conn.execute(
        ...
        if months >= 3 and months % 3 == 0:  # quarterly milestones (detectors.py:984)
            ...
            n += cal_event(
                "join_anniversary",
                f"join_anniversary:{r['tag']}:{today_chicago}",
                r["tag"],
                {
                    "name": r["name"],
                    "years": months // 12,
                    "months": months,
                    "is_annual": months % 12 == 0,
                },
            )
